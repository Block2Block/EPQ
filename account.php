...
<head>
	<meta charset="utf-8">
	<title>Account Details | EPQ Web Design Demo</title>
...
<body>
  		...
  		<div class="container-fluid">
    		...
    		<ul class="nav navbar-nav navbar-right">
				...
				if (login_check($mysqli)) :?>
					<li class="dropdown active" style="font-family: Helvetica;">
        			...
        			<span class="caret"></span></a>
        			<ul class="dropdown-menu">
						<li class="active"><a href="account"><span class="glyphicon glyphicon-user"></span> Account Details</a></li>
	...
	<div class="jumbotron text-center bg" style="background-image: url(http://files.block2block.me/uploads/jw8u8I9m3L.jpg); background-size: cover; height: 100%; background-position: center;">
    	...
    	<p class="lead">View and change account details.</p>
		<br>
		<br>
		<br>
		<br>
			...
			<div class="col-sm-8 col-xs-12">
				...
				<?php if (login_check($mysqli)): ?>
					...
					<div id="alerts">
						<?php
						if (isset($_POST['p'])) {
							change_pass(get_name($_SESSION['user_id'],$mysqli), $_POST['p'], $mysqli);
						} elseif (isset($_POST['new'])) {
							change_name(get_name($_SESSION['user_id'],$mysqli), $_POST['new'], $mysqli);
						}
						
						?>
					</div>
				
					<form action="" method="post" name="new_name" style="font-family=''">
						<fieldset style="font-family: 'Helvetica';">
    					<legend style="font-family: 'Helvetica';">Change Name</legend>
						<p style="font-size: 17px">Your current name is: <u><?php echo(get_name($_SESSION['user_id'],$mysqli));?></u></p>
						New Name: <input type="text" name="new" required="true"></input><br>
						Confirm New Name: <input type="text" name="conf" required="true"></input><br>
            			<input type="button" 
                   			value="Change Name" onclick="checknewname(this.form, this.form.new);" /> 
        			</form>
					<br>
					<br>
					<form action="" method="post" name="new_password" style="font-family=''">
						<fieldset style="font-family: 'Helvetica';">
    					<legend style="font-family: 'Helvetica';">Change Password</legend>
						New Password: <input type="password" name="new" required="true"></input><br>
						Confirm New Password: <input type="password" name="conf" required="true"></input><br>
            			<input type="button" 
                   			value="Change Password" onclick="checknewpass(this.form);" /> 
        			</form>
				
				
				 <?php else:  ?>
					<p style="font-size: 17px">You are not logged in.</p>
					<p style="font-size: 17px">Click <a href="login">here</a> to log in to a registered account, or click <a href="register">here</a> to register for one.</p>
				<?php endif; ?>
			</div>
